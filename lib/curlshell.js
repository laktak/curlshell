...
function start(config, basedir) {
  ...
  if (users) {
    ...
    server = server.bind(
      null,
      auth.digest({ realm: realm }, function(username, callback) {
  ...
  server(function(req, res) {
    ...
    var exec = commands.filter(function(x) {
      return cmd.substr(0, x.path.length) === x.path;
    })[0]; // todo/es6 use .find
    if (exec)
      exec.exec(req, res, cmd.substr(exec.path.length, cmd.length - exec.path.length - 1), args);
    else {
      res.writeHead(404);
      res.end("Unknown.");
    }
