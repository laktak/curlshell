...
module.exports=function(config, cmd, status, req, res, arg, args) {
  ...
  function stat(text) { status(new Date().toJSON()+" "+req.connection.remoteAddress+" "+cmd.name+" "+text); }

  var contentType, defaultContentType="text/plain";
  var encoding=cmd.encoding||"utf8";
  var isStreaming=cmd.stream;
  var mergeErr=false, dumpErr=false;
  var isOpen=true;
  var out="", err="";
  ...
  env.ARG=arg;
  Object.keys(args).forEach(function(key){ env["ARG_"+key.toUpperCase()]=args[key]; });
  ...
  var spawn=child_process.spawn("bash", ["-c", cmd.cmd], { cwd: cmd.cwd, env: env, encoding: encoding });
